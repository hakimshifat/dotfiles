...
Singleton {
    ...
    function next(darkMode = Appearance.m3colors.darkmode) {
        if (folderModel.count === 0) return
        if (Config.options?.background?.autoChange?.random ?? true) {
            root.randomFromCurrentFolder(darkMode)
            return
        }

        const currentPath = root.effectiveWallpaperPath
        let index = -1
        ...
        const nextIndex = (index + 1) % folderModel.count
        const filePath = folderModel.get(nextIndex, "filePath")
        root.select(filePath, darkMode)
    }

    Timer {
        id: autoChangeTimer
        interval: (Config.options?.background?.autoChange?.interval ?? 30) * 60 * 1000
        running: Config.options?.background?.autoChange?.enabled ?? false
        repeat: true
    ...
    IpcHandler {
        ...
        function setDirectory(path: string): void { root.setDirectory(path) }
    }
