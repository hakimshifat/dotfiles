...
output_json() {
  current="$(get_current)"
  case "$current" in
    performance) icon="$ICON_PERF"; klass="performance";;
    balanced)    icon="$ICON_BAL";  klass="balanced";;
    power-saver) icon="$ICON_SAVE"; klass="power-saver";;
    *)           icon="$ICON_BAL";  klass="unknown";;
  esac
...
case "$1" in
  toggle) cycle; output_json ;;
  menu)   menu;  output_json ;;
  *)      output_json ;;
esac
